from tkinter import CENTER, Button, Frame, IntVar, Label, Radiobutton, Tk, messagebox
...
class UserActions:
    def __init__(self, master, email:Email.Email):
        ...
        self.question = Label(master, text = 'What to do ?', font = ("Century Gothic", 18), bg = 'orange')
        self.question.pack()

        # radio buttons
        selectedRadio = IntVar(name='radio_value')
        self.R1 = Radiobutton(master, text="Compose Email", variable=selectedRadio, value=1, bg='orange', borderwidth=0, highlightthickness=0)
        self.R1.pack(anchor = CENTER)
        self.R2 = Radiobutton(master, text="Load a draft", variable=selectedRadio, value=2, bg='orange', borderwidth=0, highlightthickness=0)
